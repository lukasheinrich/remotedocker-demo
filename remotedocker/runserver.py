...
def start_server(publishport, container, command):
    ...
    print "start docker"
    p = subprocess.Popen(shlex.split('docker run -it -v {}:/output {} {}'.format(afsdirmount,container,command)), stdin = slave, stdout = slave, stderr = slave)
...
if __name__ == '__main__':
    start_server(5556, 'python:2.7', 'bash')
